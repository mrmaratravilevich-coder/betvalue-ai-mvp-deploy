"""Async client for API-SPORTS Hockey v1."""
...
class ApiHockeyError(RuntimeError):
    pass


async def _get(endpoint: str, params: dict | None = None) -> dict:
    if not settings.API_SPORTS_KEY:
        raise ApiHockeyError("API_SPORTS_KEY is not configured")
    url = f"{settings.API_SPORTS_HOCKEY_BASE_URL.rstrip('/')}/{endpoint.lstrip('/')}"
    ...
    if errors:
        raise ApiHockeyError(f"API-SPORTS Hockey error: {errors}")
    return data
