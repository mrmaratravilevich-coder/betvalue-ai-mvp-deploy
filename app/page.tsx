    ...
    <main>
      ...
      <section className="quality-section" aria-labelledby="quality-title">
        <div className="quality-heading">
          <div>
            <span>РЕЗУЛЬТАТЫ АНАЛИТИКИ</span>
            <h2 id="quality-title">Проверяем расчёты после финального свистка</h2>
          </div>
          <div className="quality-periods" role="group" aria-label="Период проверки">
            {[7, 30, 90].map((days) => (
              <button
                key={days}
                type="button"
        ...
        <div className="quality-board" aria-live="polite">
          <div className="quality-main-stat">
            <small>СОВПАДЕНИЕ С ИТОГОМ</small>
            <strong>
            ...
            <p>
              {selectedQuality?.overall.evaluated_matches
                ? `Проверено ${countLabel(selectedQuality.overall.evaluated_matches)} за выбранный период.`
                : "Результаты появятся после завершения первых матчей."}
            </p>
          </div>
          <div className="quality-detail">
            <span>Что это значит</span>
            <p>Мы возвращаемся к опубликованным расчётам и сопоставляем их с фактическим итогом. Так видно, как аналитика ведёт себя на реальной дистанции.</p>
          </div>
